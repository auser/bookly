...
module.exports = function (options) {
  ...
  var inputFolder = path.join(__cwd, config.output, 'mdx');
  ...
  */
  ...
  var readMdFiles = function (done) {
    utils.readMdFiles(inputFolder, config, done);
  };
  ...
  */
  var processEachFile = function (file, outPath, mainHtml, done) {
    ...
    function (pageTitle, callback) {
      ...
      var newPath = path.join(outPath, file.folder, file.name.replace(/\.md$|\.markdown$/, '-content.html'));
    ...
    function (data, callback) {
      ...
      var inputPathRegex = new RegExp(path.join(config.output, 'html') + '/(.*)');
      if (data.newPath.match(inputPathRegex)) {
      ...
      var newPublicPath = function () {
        ...
        if (!nestCount) {
          ...
          for (var i = 0; i < nestCount - 1; i++ ) { publicPath += '../'; }
        }
  ...
  */
      ...
      function (files, mainHtml, callback) {
        ...
        async.each(files, function(file, callback) {
          ...
          if( err ) { console.log('A file failed to process', err); }
            ...
            // __blogger.log('Finished convreting all the chapters.');
  ...
  async.waterfall([
    ...
    function (err) {
      if (err) { console.log(err); }
      else { __blogger.log('Finished converting each chapter!'); }
    }
  )
};
